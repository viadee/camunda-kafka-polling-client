...
import static de.viadee.camunda.kafka.pollingclient.job.runtime.RuntimeDataPollingServiceTest.PointOfTime.AFTER_TIMESLICE;
import static de.viadee.camunda.kafka.pollingclient.job.runtime.RuntimeDataPollingServiceTest.PointOfTime.BEFORE_CUTOFF;
import static de.viadee.camunda.kafka.pollingclient.job.runtime.RuntimeDataPollingServiceTest.PointOfTime.CUTOFF_TIME;
import static de.viadee.camunda.kafka.pollingclient.job.runtime.RuntimeDataPollingServiceTest.PointOfTime.END_TIME;
import static de.viadee.camunda.kafka.pollingclient.job.runtime.RuntimeDataPollingServiceTest.PointOfTime.LONG_AFTER_TIMESLICE;
import static de.viadee.camunda.kafka.pollingclient.job.runtime.RuntimeDataPollingServiceTest.PointOfTime.START_TIME;
import static de.viadee.camunda.kafka.pollingclient.job.runtime.RuntimeDataPollingServiceTest.PointOfTime.WITHIN_PAST_TIMESLICE;
import static de.viadee.camunda.kafka.pollingclient.job.runtime.RuntimeDataPollingServiceTest.PointOfTime.WITHIN_TIMESLICE;
...
public class RuntimeDataPollingServiceTest {
    ...
    enum PointOfTime {
        ...
        // @formatter:off
        BEFORE_CUTOFF(10, 0, 0), CUTOFF_TIME(10, 10, 0), WITHIN_PAST_TIMESLICE(10, 15, 0), START_TIME(10, 20, 0),
        WITHIN_TIMESLICE(10, 25, 0), END_TIME(10, 30, 0), AFTER_TIMESLICE(10, 35, 0), LONG_AFTER_TIMESLICE(10, 40, 0);
        // @formatter:on
    ...
    void setup() {
        ...
        processEngine = ProcessEngineConfiguration.createStandaloneInMemProcessEngineConfiguration()
                .setJobExecutorActivate(false).setHistory(ProcessEngineConfiguration.HISTORY_FULL)
    ...
    void pollFinishedProcessInstances(PointOfTime processStart, PointOfTime processEnd, boolean shouldBePolled) {
        ...
        processEngine.getRepositoryService().createDeployment().addClasspathResource("bpmn/simpleProcess.bpmn")
                .deploy();
        ...
        processEngine.getRuntimeService().createMessageCorrelation("pauseMessage")
        ...
        final List<String> polledProcessIds = processInstanceEventCaptor.getAllValues().stream()
                .filter(event -> event instanceof ProcessInstanceEvent).map(HistoryEvent::getId).collect(toList());
    ...
    static Stream<Arguments> pollFinishedProcessInstances() {
        ...
        return Stream.of(
                ...
                arguments(BEFORE_CUTOFF, CUTOFF_TIME, false), arguments(BEFORE_CUTOFF, WITHIN_PAST_TIMESLICE, false),
                arguments(BEFORE_CUTOFF, START_TIME, false), arguments(BEFORE_CUTOFF, WITHIN_TIMESLICE, false),
                arguments(BEFORE_CUTOFF, END_TIME, false), arguments(BEFORE_CUTOFF, AFTER_TIMESLICE, false),

                arguments(CUTOFF_TIME, WITHIN_PAST_TIMESLICE, false), arguments(CUTOFF_TIME, START_TIME, true),
                arguments(CUTOFF_TIME, WITHIN_TIMESLICE, true), arguments(CUTOFF_TIME, END_TIME, false),
                ...
                arguments(START_TIME, WITHIN_TIMESLICE, true), arguments(START_TIME, END_TIME, true),
                arguments(START_TIME, AFTER_TIMESLICE, true),

                arguments(WITHIN_TIMESLICE, END_TIME, true), arguments(WITHIN_TIMESLICE, AFTER_TIMESLICE, true),
    ...
    void pollFinishedActivitiesOfUnfinishedProcess(PointOfTime processStart, PointOfTime activityStart,
        ...
        // create testdata
        setCurrentTime(BEFORE_CUTOFF);
        processEngine.getRepositoryService().createDeployment().addClasspathResource("bpmn/activityTestProcess.bpmn")
                .deploy();
        ...
        processEngine.getRuntimeService().createMessageCorrelation("pause1").processInstanceId(processInstance.getId())
                .correlate();

        // send pause2 message on activityEnd to resume and end pause2 activity on given
        // time
        setCurrentTime(activityEnd);
        processEngine.getRuntimeService().createMessageCorrelation("pause2").processInstanceId(processInstance.getId())
                .correlate();
        ...
        assertEquals(shouldBePolled ? 1 : 0,
                historyEventCaptor.getAllValues().stream().filter(event -> event instanceof ActivityInstanceEvent)
    ...
    void pollUnfinishedActivitiesOfUnfinishedProcess(PointOfTime processStart, PointOfTime activityStart,
        ...
        assertEquals(shouldBePolled ? 1 : 0,
                historyEventCaptor.getAllValues().stream().filter(event -> event instanceof ActivityInstanceEvent)
                        .map(event -> (ActivityInstanceEvent) event)
                        .filter(event -> "pause2".equals(event.getActivityId())
                                && activityStart.date.equals(event.getStartTime()) && event.getEndTime() == null)
                        .count());
    }
    ...
    static Stream<Arguments> pollUnfinishedActivitiesOfUnfinishedProcess() {
        ...
        return Stream.of(
                ...
                arguments(BEFORE_CUTOFF, BEFORE_CUTOFF, false), arguments(BEFORE_CUTOFF, WITHIN_TIMESLICE, false),

                arguments(CUTOFF_TIME, CUTOFF_TIME, false), arguments(CUTOFF_TIME, WITHIN_TIMESLICE, true),
    ...
    static Stream<Arguments> pollUnfinishedProcessInstances() {
        ...
        return Stream.of(
                ...
                arguments(BEFORE_CUTOFF, false), arguments(CUTOFF_TIME, false), arguments(WITHIN_PAST_TIMESLICE, false),
                arguments(START_TIME, true), arguments(WITHIN_TIMESLICE, true), arguments(END_TIME, false),
    ...
    public void pollTaskComments(PointOfTime processStart, boolean shouldBePolled) {
        ...
        processEngine.getRepositoryService().createDeployment()
                .addModelInstance(processId+".bpmn", modelInstance).deploy();
        ...
        Set<String> actualMessages = taskService.getTaskComments(taskId).stream()
                .map(Comment::getFullMessage).collect(toSet());
        ...
        verify(eventSendService, atLeast(shouldBePolled ? 1 : 0)).sendEvent(commentEventCaptor.capture());

        final Set<String> polledMessages = commentEventCaptor.getAllValues().stream()
                .filter(event -> event instanceof CommentEvent).
                        map(historyEvent -> ((CommentEvent) historyEvent).getMessage()).collect(toSet());
